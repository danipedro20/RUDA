    ...
    <section class="contenido">
        ...
        <script>
            function generar_calendario(mes, anio)
            {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo base_url(); ?>assets/agenda/agenda_profesor.php",
                    cache: false,
                    data: {
            ...
            $(document).ready(function()
            {
                ...
                $(document).on("click", 'a.add', function(e)
                {
                    ...
                    $('#mask').fadeIn(1000).html("<div id='nuevo_evento' class='window' rel='" + fecha + "'>Agregar un evento el " + formatDate(fecha) + "</h2><a href='#' class='close' rel='" + fecha + "'>&nbsp;</a><div id='respuesta_form'></div><form class='formeventos'><label name='evento_nombre'>Evento</label><input type='text' name='evento_titulo' id='evento_titulo' class='required'  maxlength='100'><label name='prioridad_nombre'>Para</label><SELECT  name='prioridad' id='prioridad' style='width: 460px;'><OPTION VALUE='1'>Privado</OPTION><OPTION VALUE='2'>Aula</OPTION><OPTION VALUE='3'>Administrador</OPTION> class='required'></SELECT><br><label name='aula_nombre'>Aula</label><select name='aula' id='aula'><?php foreach ($arrDatos as $dpto) : ?><option value=" +<?php echo $dpto->idaula
        ?> + "><?php echo $dpto->aul_denominacion ?></option><?php endforeach; ?><OPTION VALUE=''>Todas</OPTION></select><input type='button' name='Enviar' value='Guardar' class='enviar'><input type='hidden' name='evento_fecha' id='evento_fecha' value='" + fecha + "'><input type='hidden' name='usuario' id='usuario' value='" + '<?php echo $this->session->userdata('id') ?>' + "'></form></div>");
                });
