...
switch ($_GET["accion"]) {
    case "listar_evento": {
                    $query = $db->query("select * from tcalendario where (fecha='" . $_GET["fecha"] . "' and idusuario='" . $_GET["usuario"] . "')  or ( fecha='" . $_GET["fecha"] . "' and prioridad= 3) ");
                    if ($fila = $query->fetch_array()){
                do {

                    echo "<p>" . $fila["evento"] . "<a href='#' class='eliminar_evento' rel='" . $fila["idagenda"] . "' title='Eliminar este Evento del " . fecha($_GET["fecha"]) . "'><img src='http://" . $_SERVER['HTTP_HOST'] . "/assets/agenda/images/delete.png'></a></p>";
    ...
    case "guardar_evento": {
            if (empty($_GET['aula'])) {
                $query2 = $db->query("select * from aulas");

                do {
                    $query = $db->query("insert into " . $tabla . " (fecha,evento,prioridad,idusuario,idaula) values ('" . $_GET["fecha"] . "','" . strip_tags($_GET["evento"]) . "','" . strip_tags($_GET["prioridad"]) . "','" . strip_tags($_GET["usuario"]) . "','" . strip_tags($fila["idaula"]) . "')");
    ...
    case "borrar_evento": {
            $query = $db->query("select *  from " . $tabla . " where idagenda='" . $_GET["id"] . "' ");
            $fila = $query->fetch_array();
            if ($fila["idusuario"] === $_GET["usuario"]) {
                $query = $db->query("delete from " . $tabla . " where idagenda='" . $_GET["id"] . "'");
    ...
    case "generar_calendario": {
            ...
            $eventos = array();
            $query = $db->query("select fecha,count(idagenda) as total from " . $tabla . " where month(fecha)='" . $fecha_calendario[1] . "' and year(fecha)='" . $fecha_calendario[0] . "' group by fecha");
            if ($fila = $query->fetch_array()) {
                do {
