...
switch ($_GET["accion"]) {
    case "listar_evento": {
            $query2 = $db->query("select DISTINCT  au.idaula from aulas as au join plan_estudios
on au.idplan=plan_estudios.idplan join cate_plan on
...
usu_cate on usu_cate.idcatedra=catedras.idcatedra join usuarios on usu_cate.idusuario=usuarios.idusuario where usuarios.idusuario='" . $_GET["usuario"] . "'");
            if ($fila1 = $query2->fetch_array()) {
                ...
                } while ($fila1 = $query2->fetch_array());
                break;
            }
        }
    case "guardar_evento": {
